...
def is_ecg_image(image_path):
    try:
        ...
        # A threshold based on typical LBP patterns of ECG vs. other images
        if hist[0] > 0.5:  # The threshold may need to be tuned based on your dataset
            return False  # High uniformity may indicate a non-ECG image
        
        # Fourier Transform to check for periodic waveforms
        f_transform = np.fft.fft2(gray_image)
        f_shift = np.fft.fftshift(f_transform)
        magnitude_spectrum = 20 * np.log(np.abs(f_shift))
        
        # Check for periodic signals (ECG typically shows periodic patterns)
        if np.mean(magnitude_spectrum) < 10:  # Adjust threshold based on testing
            return False
